...
public:
  int minPatches(vector<int>& nums, int n) {
    long miss = 1, cnt = 0, i = 0; //miss is actually the candidate that we want to test whehter it's missed
    while (miss <=n) {
	...
	miss = nums[i] + miss; //if the "missing number" is larger than the current nums[i], then it's not
	//actually missd, the next we want to test is
	++i;
      }
      else {
	miss += miss;
